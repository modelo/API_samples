...
const viewer1 = new Modelo.View.Viewer3D("model1");
viewer1.loadModel(modelId1, progress => {
    ...
    const keyboard = new Modelo.View.Input.Keyboard(viewer1);
...
const modelId2 = "aYeBJe8M";
const viewer2 = new Modelo.View.Viewer3D("model2");
viewer2.loadModel(modelId2, progress => {
    // second parameter is an optional progress callback
    const c = document.getElementById("progress");
    c.innerHTML = "Loading: " + Math.round(progress * 100) + "%";
}).then(() => {
    ...
    // add keyboard callback.
    const keyboard = new Modelo.View.Input.Keyboard(viewer2);
